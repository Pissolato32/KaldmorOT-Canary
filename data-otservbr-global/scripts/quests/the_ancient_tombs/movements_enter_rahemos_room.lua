...
function movements_enter_omruc_room.onStepIn(creature, item, position, fromPosition)
	...
	local destination = Position(33086, 32781, 14)
	if player:getStorageValue(Storage.Quest.U7_4.TheAncientTombs.DefaultStart) ~= 1 then
		...
		if player:getStorageValue(Storage.Quest.U7_4.TheAncientTombs.RahemosTreasure) <= 1 then
			player:setStorageValue(Storage.Quest.U7_4.TheAncientTombs.RahemosTreasure, 2)
		end
	end
...
movements_enter_omruc_room:uid(40089)
